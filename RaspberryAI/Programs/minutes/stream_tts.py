...
def synthesize_text(speech_lang, voice_name, text):
#for dt in ttsdt:
    ...
    voice = tts.types.VoiceSelectionParams(language_code=speech_lang,name=voice_name)
...
def listen_print_loop(responses, speech_lang):
    ...
    for response in responses:
        ...
        if not result.is_final:
            ...
            wavenet     = speech_lang + "-Wavenet-A"

            synthesize_text(speech_lang, wavenet, transcript)
...
def main():
    ...
    lang = ""
    speech_lang = lang if lang else default_lang
    language_code = speech_lang #'ja-JP' #'en-US'  # a BCP-47 language tag
    ...
    with MicrophoneStream(RATE, CHUNK) as stream:
        ...
        listen_print_loop(responses, speech_lang)
