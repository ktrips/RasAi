...
dir_image   = '/home/pi/Programs/vision/image/'
...
def send_line(message, fname):
    ...
    token = "e0SWNsumXVk6BtXd8cFXhPJixmITG6VzsmvNgPgHh7H" #ここにトークンを入力
    headers = {"Authorization" : "Bearer "+ token}

    message = "写真を撮りました！" if message == "" else message
...
def detect_faces(path):
    ...
    for face in faces:
        ...
        smile = likelihood_name[face.joy_likelihood]
        if smile in ['VERY_LIKELY', 'LIKELY', 'POSSIBLE']:
            smile_text = '笑顔を発見しました！'
            print(smile_text)
            os.system(dir_aquest + 'AquesTalkPi -g {} {} | aplay -D plughw:{},{}'.format(VOLUME, smile_text, CARD, DEVICE))
            send_line(message+smile_text, path)
    ...
    for i, label in enumerate(labels):
      if label.score > 0.8:
       if i < default_max:
        ...
        os.system(dir_aquest + 'AquesTalkPi -g {} {} | aplay -D plughw:{},{}'.format(VOLUME, trans_text, CARD, DEVICE))
        
        trans_concat += str(i) + "." + trans_text + " "
    print(trans_concat)
    send_line(trans_concat, path)
...
def run_local(args):
    ...
    # if path parameter is camera then take a picture by picamera
    if args.path == 'camera':
        args.path = camera()
        print(args.path)
